...
def get_fitness(population, chessBoard, verbose=False):
    fitness_values = np.empty((1), dtype='int64')
    for indi in population:
        if verbose: print('individo no get_fitness', indi)
...
def moreAdapted(population, chessBoard):
    ...
    fitness_values = get_fitness(population, chessBoard)
...
def do_iterations(n_iters, n_queens, chessBoard_dimension, verbose=False):
    ...
    while i <= n_iters:
        ...
        fitness_values = get_fitness(AllIndividuals, chessBoard)
        A = AllIndividuals
